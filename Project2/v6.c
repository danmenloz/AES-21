...
MODULE_DESCRIPTION("ECE 785 Project 2: I/O Response Time Analsys");
MODULE_VERSION("0.1");
...
static int __init erpi_gpio_init(void){
   ...
   gpio_direction_output(gpioLED, 0);       // set in output mode and off
...
static irq_handler_t erpi_gpio_irq_handler(unsigned int irq,
   ...
   gpio_set_value(gpioLED, 1);     // set LED
   // for(i=0;i<1000;i++){}      // make pulse wider, transition is so fast!
   gpio_set_value(gpioLED, 0);     // clear LED 
